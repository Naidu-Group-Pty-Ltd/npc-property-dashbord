...
| **C6.3** Use the selected template's typography and colours consistently, including the risk components. | **partly done** — the risk page is drawn on the template's own ledger treatment, so it now wears the colourway, and the exposure / evidence separation is kept as two columns. The shared `risk-register` block's chips are still a fixed palette in `_chips.html.ts` that reads no token and ignores `radius: '0'` — **open · S3**. |
