...
| Parties, ownership & evidence: questionnaire reconciliation + case ownership surface (V2 directive Phase 6) | V2-6 | `aml.entities`, `aml.beneficial_owners`, `aml.authorised_representatives`, `aml.entity_case_links` (existing engine), `aml.field_provenance` (append-first source record) | `aml-entities` (`import_from_questionnaire` — fill-blanks / flag-conflicts / never overwrite; `list_provenance` case-scoped read); `aml-finance` `duplicate_document_refs` surfaced in workspace | `OwnershipControlTab` (entity summary, owners/reps tables, completeness + PEP/sanctions warnings, conflicts panel, import action); workspace Documents & Evidence reused-document scan | `amlPortalContracts.test.ts` (Phase 6 block: write-role gate, no-silent-overwrite, provenance attribution, idempotency, portal isolation) |
