...
**A credential a clone cannot be given is brokered, not forwarded** — there is
no keyless path to the listing portals (`r.jina.ai` answers HTTP 200 with an
"Access Denied" body for both, measured), so `pageReadRoute.pure.ts` sends the
CALL to Mission Control the way `AIRTABLE_TOKEN` and the Didit key already do,
and the eight-host allow-list lives in that one module because a broker must
enforce it rather than trust its caller. And **a feature the migrations have
