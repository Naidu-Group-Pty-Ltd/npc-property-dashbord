...
export default function GeneratedReports() {
  ...
  const fetchInvestmentReportDetails = async (reportId: string): Promise<InvestmentReport | null> => {
    const { data, error } = await invokeSecureFunction('get-investment-reports', {
      reportId,
      listOptions: {
        select: 'id, property_address, property_listing_id, canonical_property_key, report_content, sources_content, created_at, current_version, report_scope, report_tier, parent_report_id, status, manual_overrides, report_variant, derived_from_report_id, financial_calculations, demographics_data, economic_data, investment_score, location_intelligence'
      }
    });
  ...
  const fetchInvestmentReports = async () => {
    try {
      ...
      const listOptions: Record<string, unknown> = {
        select: 'id, property_address, property_listing_id, canonical_property_key, created_at, current_version, report_scope, report_tier, parent_report_id, status, is_archived, report_variant, derived_from_report_id, investment_score, generated_by',
