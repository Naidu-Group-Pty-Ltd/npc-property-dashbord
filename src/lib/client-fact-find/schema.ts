...
export const advancedClientCreationSchema=z.object({templateVersion:z.string().min(1),branding,applicants:z.array(applicant).min(1).max(2),addresses:z.array(address).max(4),employment:z.array(employment).min(1).max(2),
 assets:z.array(asset).length(ASSET_ROW_COUNT),liabilities:z.array(liability).length(LIABILITY_ROW_COUNT),expenses:z.array(expense).length(EXPENSE_ROW_COUNT),calculatedTotals:totals.optional(),syncToGhl:z.boolean().optional()}).strict().superRefine((v,ctx)=>{
