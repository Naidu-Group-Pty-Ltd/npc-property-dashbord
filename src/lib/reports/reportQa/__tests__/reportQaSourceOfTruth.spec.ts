 ...
 *
 * The same guard the six formats before it carry, with one relaxation:
 * `../text.pure.ts`, where `neutraliseUrls` lives now that two formats need it.
 * That is a **file** and not a directory, which is what stops it reading as
 * "the parent directory is open now" — the assertion at the bottom pins it.
 *
...
/** Siblings, the design system next door, or the shared text helpers. */
const ALLOWED_IMPORT =
  /^(?:\.\/[\w.]+\.pure\.ts|\.\.\/\.\.\/reportDesign\/[\w.]+\.(?:pure|generated)\.ts|\.\.\/text\.pure\.ts)$/;
...
describe('report Q&A — single source of truth', () => {
      ...
      for (const spec of imports) {
        expect(
          ...
          `${file} imports "${spec}" — a module here may import its siblings, `
            + '`../../reportDesign/*.pure.ts` and `../text.pure.ts`, and nothing else',
  ...
  it('permits the shared text helpers and no other format', () => {
    ...
    expect(pattern).toMatch(/text\\?\.pure\\?\.ts/);
