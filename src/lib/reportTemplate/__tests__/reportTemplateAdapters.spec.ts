...
describe('report template adapter registry', () => {
  ...
  it('supports production for the report types that have a real data source', () => {
    ...
    // 143 real rows. This list is the gate `deriveEntryFacts` reads for
    // `production_ready`, so adding a type to it without a working
    // `buildBindingContext` marks templates report-ready that cannot render one.
    for (const reportType of ['investment', 'borrowing_capacity']) {
  ...
  it('marks the remaining report types preview-only until adapters are implemented', () => {
    const previewOnlyTypes = ['portfolio', 'cashflow', 'qa', 'suburb', 'postcode', 'statewide', 'comparison', 'formara'];

    expect(listAdapters().map((adapter) => adapter.reportType))
      .toEqual(expect.arrayContaining(['investment', 'borrowing_capacity', ...previewOnlyTypes]));
