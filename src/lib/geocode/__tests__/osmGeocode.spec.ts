...
describe('the shared shape', () => {
  ...
  it('parses the provider order and defaults to no Google', () => {
    expect(parseProviderOrder(undefined)).toEqual(['nominatim', 'photon', 'abs_locality']);
    expect(parseProviderOrder('')).toEqual(['nominatim', 'photon', 'abs_locality']);
    ...
    expect(parseProviderOrder('bing')).toEqual(['nominatim', 'photon', 'abs_locality']);
  });
