...
export function LivingExpensesTab() {
  ...
  return <div className="space-y-3" data-testid="advanced-expenses-tab">
    <header className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-card p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
    ...
    <section data-testid="expense-continuous-list" aria-label="Living expenses worksheet" className="w-full min-w-0 overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
      <div data-testid="expense-list-header" className="sticky top-0 z-10 hidden grid-cols-[minmax(0,20fr)_minmax(0,35fr)_minmax(8rem,15fr)_minmax(0,30fr)] border-b border-border bg-muted px-4 py-3 text-[10px] font-bold uppercase tracking-[0.14em] text-foreground md:grid">
      ...
      <div className="divide-y divide-border">
        {LIVING_EXPENSE_ITEMS.map((item, index) => {
          ...
          return <div
            ...
            className="grid min-w-0 grid-cols-1 gap-3 px-4 py-3 odd:bg-muted/15 transition-colors duration-150 hover:bg-primary/5 focus-within:bg-primary/10 motion-reduce:transition-none md:grid-cols-[minmax(0,20fr)_minmax(0,35fr)_minmax(8rem,15fr)_minmax(0,30fr)] md:items-center md:gap-0"
            key={item.key}
          >
            ...
            <label className="block min-w-0 text-xs font-semibold text-muted-foreground md:pr-3">
              ...
              <input aria-label={`${item.itemLabel} monthly amount`} type="number" min="0" step="0.01" className="mt-1 h-11 w-full min-w-0 rounded-lg border border-input bg-background/80 px-3 text-right text-sm tabular-nums text-foreground transition-[border-color,box-shadow] duration-150 focus-visible:border-brand-300/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 motion-reduce:transition-none md:mt-0" {...register(`expenses.${index}.monthlyAmount`, { setValueAs: value => value === '' ? 0 : Number(value) })} />
            </label>
            <label className="block min-w-0 text-xs font-semibold text-muted-foreground">
              <span className="md:sr-only">Notes</span>
              <input aria-label={`${item.itemLabel} notes`} className="mt-1 h-11 w-full min-w-0 rounded-lg border border-input bg-background/80 px-3 text-sm text-foreground transition-[border-color,box-shadow] duration-150 focus-visible:border-brand-300/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 motion-reduce:transition-none md:mt-0" {...register(`expenses.${index}.notes`)} />
            </label>
          </div>;
        })}
      </div>
