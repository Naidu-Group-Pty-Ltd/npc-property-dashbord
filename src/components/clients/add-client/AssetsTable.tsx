...
const AssetRow = memo(function AssetRow({ row, register, onRemove }: { row: number; register: UseFormRegister<AdvancedClientCreationPayload>; onRemove: () => void }) {
  return <fieldset data-testid="asset-card" className="group w-full min-w-0 max-w-full rounded-xl border border-border/70 border-l-2 border-l-success/40 bg-background/45 p-3 transition-[border-color,background-color] duration-150 focus-within:border-success/45 focus-within:bg-muted/20 motion-reduce:transition-none sm:p-4">
    <legend className="ml-1 px-1"><span className="inline-flex rounded-full border border-success/25 bg-card px-3 py-1 text-xs font-bold uppercase tracking-wider text-success">Asset {row + 1}</span></legend>
    <div className="mb-3 flex justify-end"><Button type="button" variant="ghost" size="sm" aria-label={`Remove Asset ${row + 1}`} className="h-8 text-muted-foreground hover:bg-destructive/10 hover:text-destructive focus-visible:ring-destructive/40" onClick={onRemove}><Trash2 className="mr-1.5 h-3.5 w-3.5" aria-hidden="true" />Remove Asset</Button></div>
    <input type="hidden" {...register(`assets.${row}.displayOrder`)} />
    <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">{names.map((name, column) => { const label = ASSET_COLUMNS[column].label; return <label key={name} className={`min-w-0 space-y-1.5 text-sm ${wideNames.has(name) ? 'sm:col-span-2' : ''}`}><span className="block break-words text-xs font-semibold text-foreground/90">{label}</span><input aria-label={`Asset ${row + 1} ${label}`} type={numericNames.has(name) ? 'number' : name === 'maturityDate' ? 'date' : 'text'} min={numericNames.has(name) ? 0 : undefined} step={name === 'interestRate' ? '0.0001' : numericNames.has(name) ? '0.01' : undefined} className="h-11 w-full min-w-0 max-w-full rounded-lg border border-input bg-background/80 px-3 text-sm transition-[border-color,box-shadow] duration-150 focus-visible:border-brand-300/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 motion-reduce:transition-none" {...register(`assets.${row}.${name}` as const, { setValueAs: value => numericNames.has(name) ? (value === '' ? 0 : Number(value)) : value })} /></label>; })}</div>
  </fieldset>;
});
