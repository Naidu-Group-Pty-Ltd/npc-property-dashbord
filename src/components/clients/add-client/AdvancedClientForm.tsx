...
export function AdvancedClientForm({ onCancel }: { active: boolean; onCancel: () => void }) {
  ...
  </div></div></Tabs><footer data-testid="advanced-footer" className="z-30 shrink-0 border-t border-brand-300/25 bg-card px-3 py-3 shadow-[0_-8px_24px_hsl(var(--background)/0.45)] sm:px-6"><div className="mx-auto flex max-w-[1400px] flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><p className="hidden text-xs text-muted-foreground md:block">Create a client after reviewing all Advanced form sections.</p><div className="grid w-full grid-cols-2 gap-2 sm:ml-auto sm:flex sm:w-auto sm:justify-end"><Button type="button" variant="outline" className="min-h-11 rounded-xl text-muted-foreground hover:text-foreground" onClick={onCancel} disabled={creating}>Cancel</Button>{tab!=='output'&&<Button type="button" variant="outline" className="min-h-11 rounded-xl border-brand-300/25 font-semibold hover:bg-primary/10" onClick={reviewOutput} disabled={creating}><FileCheck2 className="mr-2 h-4 w-4"/>Review Output</Button>}<Button type="button" className="dashboard-luxury-primary-cta col-span-2 min-h-11 rounded-xl px-5 font-bold transition-transform duration-150 active:scale-[0.98] motion-reduce:transition-none sm:col-auto" onClick={createClient} disabled={creating}>{creating?<Loader2 className="mr-2 h-4 w-4 animate-spin motion-reduce:animate-none"/>:<UserPlus className="mr-2 h-4 w-4"/>}{creating?'Creating Client…':'Create Client'}</Button></div></div></footer></form></FormProvider>;
}
