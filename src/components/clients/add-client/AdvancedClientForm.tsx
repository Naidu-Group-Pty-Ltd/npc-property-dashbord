...
export function AdvancedClientForm({ onCancel }: { active: boolean; onCancel: () => void }) {
  ...
  const contentRef=useRef<HTMLDivElement>(null),createdClientId=useRef<string|null>(null),queryClient=useQueryClient();
  ...
  const createClient=async()=>{
    ...
    try{
      ...
      const failed=await saveAdvancedViewClientData(clientId,payload,invokeSecureFunction);
      let attachmentFailed=false;if(workbook){try{await attachWorkbook(clientId,workbook)}catch{attachmentFailed=true}}
      ...
      if(failed.length){toast.error(`Client created, but ${failed.join(', ')} could not be saved. Open the client to review the saved information.`);return}
