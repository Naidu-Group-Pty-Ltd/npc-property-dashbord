...
import { escapePdfHtml } from '@/utils/commercial/escapePdfHtml';
...
export function TenYearCashFlowCard() {
  ...
  const buildPdfHtml = () => {
    ...
    const warnings = priorityWarnings.map(warning => `<li><strong>${escapePdfHtml(warning.category)} · ${escapePdfHtml(warning.severity)}:</strong> ${escapePdfHtml(warning.message)}</li>`).join('') || '<li>No priority warnings.</li>';
    const assumptions = [`Rent growth: ${inputs.rentGrowthPct}%`, `Vacancy allowance: ${inputs.vacancyAllowancePct}%`, `Terminal cap rate: ${inputs.terminalCapRatePct}%`, `Capital growth: ${inputs.capitalGrowthPct}%`, `Tax rate: ${inputs.taxRatePct}%`].map(item => `<li>${escapePdfHtml(item)}</li>`).join('');
    return `<!doctype html><html><head><title>10-Year Cash Flow Report</title><style>body{font-family:Arial,sans-serif;color:#111827;margin:32px}h1,h2{color:#0f172a}.cover{border-bottom:3px solid #2563eb;padding-bottom:16px;margin-bottom:24px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.card{border:1px solid #d1d5db;border-radius:8px;padding:12px;margin:8px 0}.muted{color:#6b7280}table{width:100%;border-collapse:collapse;margin-top:8px}th,td{border:1px solid #d1d5db;padding:6px;text-align:right}th:first-child,td:first-child{text-align:left}@media print{button{display:none}.page-break{break-before:page}}</style></head><body><section class="cover"><h1>10-Year Cash Flow Report</h1><p class="muted">${escapePdfHtml(scenarioName || '10-Year Cash Flow Scenario')} · ${escapePdfHtml(TEN_YEAR_REPORT_VERSION)}</p><p>${escapePdfHtml((dealProfile as any)?.address ?? (dealProfile as any)?.propertyAddress ?? 'Property address not provided')}</p></section>${pdfOptions.assumptions ? `<section><h2>Key Assumptions</h2><ul>${assumptions}</ul></section>` : ''}<section><h2>Source and Verification Summary</h2><p>Data source: ${escapePdfHtml(buildGlobalSyncLabel(sourceMode as CalculatorSourceMode))}. Readiness status: ${escapePdfHtml(reportStatus)}. Generated: ${escapePdfHtml(generatedCashFlow ? new Date(generatedCashFlow.generatedAt).toLocaleString() : PENDING)}.</p></section>${pdfOptions.summaryCards ? `<section><h2>Summary Output Cards</h2><div class="grid"><div class="card">Year 1 NOI<br/><strong>${fmt(s.year1Noi)}</strong></div><div class="card">Year 10 Property Value<br/><strong>${fmt(s.year10PropertyValue)}</strong></div><div class="card">Year 10 Equity<br/><strong>${fmt(s.year10Equity)}</strong></div><div class="card">Cumulative After-Tax Cashflow<br/><strong>${fmt(s.cumulativeAfterTaxCashflow)}</strong></div></div></section>` : ''}${pdfOptions.fullTable ? `<section class="page-break"><h2>10-Year Cashflow Table</h2><table><thead><tr><th>Year</th><th>Property Value</th><th>Actual NOI</th><th>Total Capex</th><th>After-Tax Cashflow</th><th>Equity Position</th></tr></thead><tbody>${rows}</tbody></table></section>` : ''}<section><h2>Valuation / Exit Value</h2><p>Year 10 capital growth value: ${fmt(year10CapitalGrowthValue)}. Terminal cap value: ${fmt(terminalCapValue)}. Selected method: ${escapePdfHtml(exitValueMethod)}. Selected exit value: ${fmt(selectedExitValueUsed)}.</p></section><section><h2>Debt Metrics</h2><p>Loan amount: ${fmt(inputs.loanAmount)}. Interest rate: ${inputs.interestRatePct}%. Repayment type: ${escapePdfHtml(title(inputs.repaymentType))}.</p></section><section><h2>NOI and Cashflow</h2><p>Year 1 NOI: ${fmt(s.year1Noi)}. Year 1 after-tax cashflow: ${fmt(s.year1AfterTaxCashflow)}.</p></section><section><h2>Capex and Leasing</h2><p>Annual capex reserve: ${fmt(inputs.annualCapexReserve)}. Downtime: ${inputs.downtimeMonths} months. Tenant incentives: ${inputs.incentiveMonths} months.</p></section>${pdfOptions.warnings ? `<section><h2>Warnings and Assumptions</h2><ul>${warnings}</ul></section>` : ''}${pdfOptions.scenarioNotes ? `<section><h2>Scenario Notes</h2><h3>Base case</h3><p>${escapePdfHtml(scenarioNotes.base)}</p><h3>Conservative case</h3><p>${escapePdfHtml(scenarioNotes.conservative)}</p><h3>Upside case</h3><p>${escapePdfHtml(scenarioNotes.upside)}</p><h3>Manual scenario note</h3><p>${escapePdfHtml(scenarioNotes.manual || 'No manual scenario note provided.')}</p></section>` : ''}${pdfOptions.commentary ? `<section><h2>Report Commentary</h2><p>${escapePdfHtml(editableCommentary || generatedCommentary || reportResult.commentary)}</p></section>` : ''}${pdfOptions.aiEstimateNotes ? `<section><h2>AI Estimate Notes</h2><p>AI/research estimates, if accepted, remain subject to verification and review before client reliance.</p></section>` : ''}${pdfOptions.formulaNotes ? `<section><h2>Formula Notes</h2><p>Calculated rows are protected. Annual overrides affect assumptions only where explicitly applied.</p></section>` : ''}<section><h2>Disclaimer / Estimate Note</h2><p>This report is an estimate and should be reviewed against lease documents, tax/GST advice, lender terms and market evidence before client reliance.</p></section></body></html>`;
  };
