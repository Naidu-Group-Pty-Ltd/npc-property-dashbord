...
import { escapeHtml } from '@/utils/commercial/escapeHtml';
...
import { tenYearCashFlowPrintStyle } from '@/utils/commercial/tenYearCashFlowPrintStyle';
...
function MetricRows({ years, mode, inputs, pending, annualOverridesEnabled, annualOverrides, onSetAnnualOverride, onResetAnnualOverride, onResetAnnualOverrideRow }: { years: TenYearCashFlowYear[]; mode: TenYearCashFlowMode; inputs: TenYearCashFlowInputs; pending?: boolean; annualOverridesEnabled: boolean; annualOverrides: TenYearAnnualOverrides; onSetAnnualOverride: (field: TenYearAnnualOverrideField, year: number, value: number, originalValue: number) => void; onResetAnnualOverride: (field: TenYearAnnualOverrideField, year: number) => void; onResetAnnualOverrideRow: (field: TenYearAnnualOverrideField) => void }) {
  ...
  const renderCell = (row: MetricRowConfig, year: TenYearCashFlowYear) => {
    ...
    return <div className="flex min-w-[150px] flex-col items-end gap-1">
      ...
      {override && <div className="max-w-[180px] text-right text-[10px] text-muted-foreground"><Badge variant="secondary" className="mb-1 text-[10px]">Override</Badge><div>Original: {originalValue.toLocaleString('en-AU')}</div><div>New: {override.value.toLocaleString('en-AU')}</div><div>{override.user ?? 'Current user'} · {new Date(override.timestamp).toLocaleString('en-AU')}</div><Button type="button" size="sm" variant="ghost" className="h-6 px-2 text-[10px]" onClick={() => onResetAnnualOverride(row.overrideField!, year.year)}>Reset this cell</Button></div>}
    </div>;
  };
...
export function TenYearCashFlowCard() {
  ...
  const buildPdfHtml = (design: DrawnDocumentDesign | null = null) => {
    const safe = escapeHtml;
    const rows = reportResult.years.map(year => `<tr><td>${safe(year.year)}</td><td>${safe(fmt(year.propertyValue))}</td><td>${safe(fmt(year.actualNoi))}</td><td>${safe(fmt(year.totalCapex))}</td><td>${safe(fmt(year.afterTaxCashflow))}</td><td>${safe(fmt(year.equityPosition))}</td></tr>`).join('');
    const warnings = priorityWarnings.map(warning => `<li><strong>${safe(warning.category)} · ${safe(warning.severity)}:</strong> ${safe(warning.message)}</li>`).join('') || '<li>No priority warnings.</li>';
    const assumptions = [`Rent growth: ${inputs.rentGrowthPct}%`, `Vacancy allowance: ${inputs.vacancyAllowancePct}%`, `Terminal cap rate: ${inputs.terminalCapRatePct}%`, `Capital growth: ${inputs.capitalGrowthPct}%`, `Tax rate: ${inputs.taxRatePct}%`].map(item => `<li>${safe(item)}</li>`).join('');
    return `<!doctype html><html><head><title>10-Year Cash Flow Report</title><style>${tenYearCashFlowPrintStyle(design)}</style></head><body><section class="cover"><h1>10-Year Cash Flow Report</h1><p class="muted">${safe(scenarioName || '10-Year Cash Flow Scenario')} · ${safe(TEN_YEAR_REPORT_VERSION)}</p><p>${safe((dealProfile as any)?.address ?? (dealProfile as any)?.propertyAddress ?? 'Property address not provided')}</p></section>${pdfOptions.assumptions ? `<section><h2>Key Assumptions</h2><ul>${assumptions}</ul></section>` : ''}<section><h2>Source and Verification Summary</h2><p>Data source: ${safe(buildGlobalSyncLabel(sourceMode as CalculatorSourceMode))}. Readiness status: ${safe(reportStatus)}. Generated: ${safe(generatedCashFlow ? new Date(generatedCashFlow.generatedAt).toLocaleString('en-AU') : PENDING)}.</p></section>${pdfOptions.summaryCards ? `<section><h2>Summary Output Cards</h2><div class="grid"><div class="card">Year 1 NOI<br/><strong>${safe(fmt(s.year1Noi))}</strong></div><div class="card">Year 10 Property Value<br/><strong>${safe(fmt(s.year10PropertyValue))}</strong></div><div class="card">Year 10 Equity<br/><strong>${safe(fmt(s.year10Equity))}</strong></div><div class="card">Cumulative After-Tax Cashflow<br/><strong>${safe(fmt(s.cumulativeAfterTaxCashflow))}</strong></div></div></section>` : ''}${pdfOptions.fullTable ? `<section class="page-break"><h2>10-Year Cashflow Table</h2><table><thead><tr><th>Year</th><th>Property Value</th><th>Actual NOI</th><th>Total Capex</th><th>After-Tax Cashflow</th><th>Equity Position</th></tr></thead><tbody>${rows}</tbody></table></section>` : ''}<section><h2>Valuation / Exit Value</h2><p>Year 10 capital growth value: ${safe(fmt(year10CapitalGrowthValue))}. Terminal cap value: ${safe(fmt(terminalCapValue))}. Selected method: ${safe(exitValueMethod)}. Selected exit value: ${safe(fmt(selectedExitValueUsed))}.</p></section><section><h2>Debt Metrics</h2><p>Loan amount: ${safe(fmt(inputs.loanAmount))}. Interest rate: ${safe(inputs.interestRatePct)}%. Repayment type: ${safe(title(inputs.repaymentType))}.</p></section><section><h2>NOI and Cashflow</h2><p>Year 1 NOI: ${safe(fmt(s.year1Noi))}. Year 1 after-tax cashflow: ${safe(fmt(s.year1AfterTaxCashflow))}.</p></section><section><h2>Capex and Leasing</h2><p>Annual capex reserve: ${safe(fmt(inputs.annualCapexReserve))}. Downtime: ${safe(inputs.downtimeMonths)} months. Tenant incentives: ${safe(inputs.incentiveMonths)} months.</p></section>${pdfOptions.warnings ? `<section><h2>Warnings and Assumptions</h2><ul>${warnings}</ul></section>` : ''}${pdfOptions.scenarioNotes ? `<section><h2>Scenario Notes</h2><h3>Base case</h3><p>${safe(scenarioNotes.base)}</p><h3>Conservative case</h3><p>${safe(scenarioNotes.conservative)}</p><h3>Upside case</h3><p>${safe(scenarioNotes.upside)}</p><h3>Manual scenario note</h3><p>${safe(scenarioNotes.manual || 'No manual scenario note provided.')}</p></section>` : ''}${pdfOptions.commentary ? `<section><h2>Report Commentary</h2><p>${safe(editableCommentary || generatedCommentary || reportResult.commentary)}</p></section>` : ''}${pdfOptions.aiEstimateNotes ? `<section><h2>AI Estimate Notes</h2><p>AI/research estimates, if accepted, remain subject to verification and review before client reliance.</p></section>` : ''}${pdfOptions.formulaNotes ? `<section><h2>Formula Notes</h2><p>Calculated rows are protected. Annual overrides affect assumptions only where explicitly applied.</p></section>` : ''}<section><h2>Disclaimer / Estimate Note</h2><p>This report is an estimate and should be reviewed against lease documents, tax/GST advice, lender terms and market evidence before client reliance.</p></section></body></html>`;
  };

  const runPdfGeneration = () => {
    const timestamp = new Date().toISOString();
    // The window is opened inside the click, where a browser allows it, and
    // filled once the design chosen for the C&I Capacity report has been read
    // (`drawnDocumentDesign.ts`) — which never throws, and is the house style
    ...
    <Dialog open={pdfPreviewOpen} onOpenChange={setPdfPreviewOpen}>
      <DialogContent className="max-w-3xl">
        ...
        <div className="space-y-3 text-sm">
          ...
          <p><span className="font-medium">Generated:</span> {generatedCashFlow ? new Date(generatedCashFlow.generatedAt).toLocaleString('en-AU') : PENDING}</p>
    ...
    <Dialog open={pdfConfirmOpen} onOpenChange={setPdfConfirmOpen}>
      <DialogContent className="max-w-3xl">
        ...
        <div className="grid gap-3 text-sm md:grid-cols-2">
          ...
          <div><span className="font-medium">Generated date:</span> {generatedCashFlow ? new Date(generatedCashFlow.generatedAt).toLocaleString('en-AU') : PENDING}</div>
