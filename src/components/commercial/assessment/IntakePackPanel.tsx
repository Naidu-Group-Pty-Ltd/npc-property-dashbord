...
import {
  ...
  Loader2, Paperclip, Trash2, Upload, UserPlus, X,
} from 'lucide-react';
...
export function IntakePackPanel({
  payload, assessmentReference, assessmentTitle, onApply, onCreateClient, disabled,
}: Props) {
    ...
    for (const file of accepted) {
      if (isPackFile(file.name)) {
        ...
        try {
          ...
          setParsed(result);
          setPackFile(file.name);
          if (!result.recognised) {
            toast({
              title: 'Not an intake pack',
      ...
      // Anything else is a supporting document.
      if (file.size > MAX_SUPPORTING_BYTES) {
        toast({
          title: 'File too large',
          description: `${file.name} is ${formatBytes(file.size)}. The limit is ${formatBytes(MAX_SUPPORTING_BYTES)}.`,
          variant: 'destructive',
        });
        continue;
      }
      counter.current += 1;
      setSupporting((current) => [...current, {
        id: `file-${counter.current}`, name: file.name, size: file.size, type: file.type,
      }]);
    }
  }, []);
          ...
          {parsing ? (
            ...
            <>
              ...
              <p className="mt-1 max-w-md text-xs leading-5 text-muted-foreground">
                The Excel workbook populates the assessment. Any other file — leases, financials,
                rates notices, identification — is kept alongside it as a supporting document.
              </p>
            </>
          )}
        </div>
