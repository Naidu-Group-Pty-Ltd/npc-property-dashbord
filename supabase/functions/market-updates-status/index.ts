  ...
  try {
    ...
    if (action === 'digest') {
      ...
      const { data, error } = await sb.from('market_digests').select(DIGEST_COLUMNS)
        .eq('status','published').eq('period',period).order('generated_at',{ ascending:false }).limit(1).maybeSingle();
    ...
    const [sourcesResult, latestRunResult, latestFetchResult, latestDigestResult, updateRows, assignmentsResult, openRouterResult, latestCronResult] = await Promise.all([
      ...
      sb.from('agent_model_assignments').select('agent_key,route,model_id,fallback_chain,last_used_at,last_error,updated_at').in('agent_key',AGENT_KEYS),
