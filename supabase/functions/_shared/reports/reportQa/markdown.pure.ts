 ...
 *
 * Six formats have been migrated onto the design system and every one of them
 * had a typed payload — `Measure {value, unit}` fields, arithmetic, tables built
 * from columns somebody declared. This is the seventh, and its payload is a
 * `text` column holding whatever a model wrote. So the format needs something
 * none of the others did: a way to turn Markdown prose into the system's own
 * HTML without either losing what the model said or letting it say anything.
 *
...
export function markdownToPlainText(value: string, maxChars?: number): string {
  const clean = neutraliseUrls(stripMarkers(sanitiseGlyphs(String(value ?? '')).text))
    ...
    .replace(/\s+/g, ' ')
    .trim();
  return typeof maxChars === 'number' ? clean.slice(0, maxChars).trim() : clean;
}
