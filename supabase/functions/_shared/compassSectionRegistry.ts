/**
 * Compass Section Registry — Shared (edge runtime)
 * ------------------------------------------------
 * Single source of truth for the Investment Location & Property Fit report's
 * structure, and for the separate Financial Analysis Report.
 *
 * `generate-investment-report` builds its section list, its per-section prompt
 * and its `total_sections` from this file; `compassPostProcessor` and
 * `compassQAValidator` enforce what it declares. A change here reaches
 * production without any renderer work, because the markdown this produces is
 * the report on every live path (see docs/reports/COVERAGE.md).
 *
 * Frontend mirror: src/lib/reports/compassSectionRegistry.ts.
 * The two are kept in step by `compassRegistryParity.spec.ts`, which compares
 * the section arrays field by field — they had drifted to 672 lines against
 * 174 with nothing checking, which docs/reports/DESIGN_SYSTEM.md records.
 */

import { riskRegisterInstruction } from './reports/investment/riskRegister.pure.ts';
...
export const COMPASS_40_SECTIONS: CompassSectionDefinition[] = [
  ...
  },
  {
    ...
    purpose: 'The section this report had no home for. Zoning and planning were sourceHeadings of the Risk Dashboard — a 500-word table whose own purpose says "the table IS the section" — so the controls retrieved for the property had nowhere to be explained and the reader got a row. Lead with the retrieved register: the zone and its instrument, the numeric controls the table lists — in the terms the jurisdiction\'s own instrument uses, never a control it does not use (Western Australia has no floor space ratio) — with the clause that creates each one, and every overlay or hazard a register returned, each with its currency date. Then explain what each control OBLIGES — not what it is called — and close with the certificate that settles it in this jurisdiction and the questions to put with it. State only what the register returned; where a register answered and found nothing, say it was checked; where none was reached, say the council scheme has not been read. An absence is never a clearance and a zone that admits a use is never approval for it. NO financial figures. OPEN with the property\'s verified identity — the lot and plan, the parcel area and its basis where the cadastre stated one, the local government area — because a planning finding is about a PARCEL and a reader cannot check one against an address alone. Where no parcel area was retrieved, say so in its own sentence ("the cadastral area of the lot was not retrieved") and never write a value slot around the absence; a land size the property record carries is that record\'s figure, is named as such, and is never presented as a cadastral measurement. Then, for every control and every land use: what was FOUND, what it MEANS for this property, the PRACTICAL implication for a buyer or a holder, and what REMAINS to be verified and where. Four moves, in that order, in ordinary sentences — never as four labelled fields. Where the land use table was retrieved it is the authority on what may be built: do not soften a prohibition into a possibility and do not infer a secondary dwelling, a dual occupancy or a subdivision from the block size, the street or the zone code. Land size is not a permission.',
  },
  {
