...
Deno.serve(async req=>{
  ...
  for(const event of events||[]){
    ...
    try{
      ...
      await db.from('integration_outbox').update({available_at:disposition.availableAt,locked_at:null,locked_by:null,last_error:message.slice(0,2000),...(terminal?{processed_at:new Date().toISOString()}:{})}).eq('id',event.id).eq('locked_by',id);
