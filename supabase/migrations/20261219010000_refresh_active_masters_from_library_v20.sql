-- @effect: select 1 from public.template_master_refresh_decisions where release = '20261219000000_seed_template_library_v20_continuous_front_matter'
...
-- lone SELECT.
-- Bring the v20 catalogue to the ACTIVE `report_templates` rows that adopted
-- from the library — and to no others.
--
-- Identical in mechanism to the v15 refresh (`20261204030000`) and to v18's
-- and v19's, which is the point: the classification, the snapshot and the
-- four verdicts are the safety property, not something to re-invent per
...
--
-- ## What v20 changes in a master, and why it is safe to carry forward
--
-- The 50 Investment Compass masters, and nothing else: the other ten
-- formats' 493 masters and the 43 voice templates are byte-identical,
-- verified by parsing both seed files and comparing all 543 schemas.
--
-- The owner read the 23 Sep 2026 Compass for 97 Poole Road and 9 Hollow
-- Street and sent pages 3 to 6 back: 54%, 68%, 66% and 74% empty, on every
-- master, because each summary element was a page of its own laid out for
-- the worst case — a KPI ledger declaring six rows for a record that
-- publishes two, a property table declaring eight guarded rows and drawing
-- five — with the report's body waiting on the page after the last of them.
--
--   * Each master gains ONE page, `Executive summary`, drawn for the Compass
--     and the four tiers condensed from it: the verdict, the two figures the
--     tier publishes, the property, the strengths and watch-points and — on
--     the Compass alone — the grade's dimensions, and then the report's body,
--     OPENING ON THE SAME PAGE in the room left under them. The page is a
--     `flow` page: the renderer stacks its blocks from what they draw rather
--     than from the heights they declare (`src/lib/reportTemplate/flowLayout.ts`).
--   * The typed pages the summary replaces — `The property`, `The
--     assessment`, `Risk and recommendation`, `Sources and methodology`, and
--     the report's own opening page — are kept in the schema and made
--     conditional on the report NOT flowing, so a stored pre-tier report
--     (the composite) prints exactly as it did. That is why every master's
--     block count rises (by 9 to 13): nothing is removed.
--
-- The geometry gate (`npm run templates:compass:qa`) draws every master
-- through this page for all five tiers — it did not before this release,
-- because its fixture did not carry the three flags that select it — and
-- all 710 renders are clean.
...
drop table if exists _v20_classified;
create temporary table _v20_classified as
with adopted as (
  select
    t.id as template_id,
    e.id as entry_id,
  ...
  left join public.template_library_release_baselines b
    on b.entry_id = e.id
   and b.release = '20261219000000_seed_template_library_v20_continuous_front_matter'
  where t.is_active
...
select
  c.template_id,
  '20261219000000_seed_template_library_v20_continuous_front_matter',
  c.entry_id,
  c.verdict,
  c.differing_keys
from _v20_classified c
...
select
  c.template_id,
  '20261219010000_refresh_active_masters_from_library_v20',
  c.row_schema,
  c.row_config,
  c.differing_keys
from _v20_classified c
...
-- nowhere else, so a deferred master can never read as though it received v20.
...
set
  ...
  config = jsonb_set(
    ...
    to_jsonb('20261219000000_seed_template_library_v20_continuous_front_matter'::text)
  ),
  updated_at = now()
from _v20_classified c
...
--   where release = '20261219000000_seed_template_library_v20_continuous_front_matter'
...
--     and s.migration = '20261219010000_refresh_active_masters_from_library_v20'
