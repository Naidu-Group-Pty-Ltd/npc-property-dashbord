-- @effect: select 1 from public.template_master_refresh_decisions where release = '20261225090000_seed_template_library_v23_issuer_tagline'
...
-- lone SELECT.
-- Bring the v23 catalogue to the ACTIVE `report_templates` rows that adopted
-- from the library — and to no others.
--
-- Identical in mechanism to the v15 refresh (`20261204030000`) and to v18's,
-- v19's, v20's, v21's and v22's, which is the point: the classification, the
-- snapshot and the four verdicts are the safety property, not something to
-- re-invent per release. Only the release identifiers change. Read v15's
-- header for why a customised master is left alone rather than overwritten,
-- and for the queries that report what this release did.
--
-- ## What v23 changes in a master, and why it is safe to carry forward
--
-- All 500 family masters, in one block each; the 43 voice templates are
-- byte-identical, verified by parsing both seed files and comparing all 543
-- rows.
--
-- Every family master set the line under the cover's wordmark as a literal,
-- "Your dedicated property partner", which is the house's own tagline, so
-- every clone's templated cover carried the house's words under the clone's
-- name. The owner's rule (26 Sep 2026): the house's identity is a legacy the
-- prime keeps and a clone never sees.
--
--   * The cover's "Tagline" text block binds `{{org.tagline}}` instead of the
--     literal, in `schema` and in `preview_schema`. Nothing else on any page
--     moves: every other block, its order and its id are unchanged.
--   * `required_bindings` gains `org.tagline`. No other column changes.
--   * `org.tagline` is published on the prime alone, with exactly the words
--     the literal carried, so the prime's covers print what they always
--     printed. A clone's cover draws no tagline: a bound text block that
--     resolves to nothing is not drawn.
--
-- A master a tenant has edited is not replaced (the classification below),
-- so an edited clone master keeps whatever tagline its tenant gave it.
...
drop table if exists _v23_classified;
create temporary table _v23_classified as
with adopted as (
  select
    t.id as template_id,
    e.id as entry_id,
  ...
  left join public.template_library_release_baselines b
    on b.entry_id = e.id
   and b.release = '20261225090000_seed_template_library_v23_issuer_tagline'
  where t.is_active
...
select
  c.template_id,
  '20261225090000_seed_template_library_v23_issuer_tagline',
  c.entry_id,
  c.verdict,
  c.differing_keys
from _v23_classified c
...
select
  c.template_id,
  '20261225100000_refresh_active_masters_from_library_v23',
  c.row_schema,
  c.row_config,
  c.differing_keys
from _v23_classified c
...
-- nowhere else, so a deferred master can never read as though it received v23.
...
set
  ...
  config = jsonb_set(
    ...
    to_jsonb('20261225090000_seed_template_library_v23_issuer_tagline'::text)
  ),
  updated_at = now()
from _v23_classified c
...
--   where release = '20261225090000_seed_template_library_v23_issuer_tagline'
...
--     and s.migration = '20261225100000_refresh_active_masters_from_library_v23'
